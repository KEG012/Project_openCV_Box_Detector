...
def is_cal_box(frame):
    ...
    if max_area_index != -1:
        ...
        cv2.line(frame_HSV, (x+dist, y-dist), (x-dist, y+dist), (0, 0, 255), 5)
        # cv2.imshow("frame_threshold", frame_HSV)
        # cv2.waitKey(0)
        ...
        cv2.imshow("frame_threshold", frame_roi)
        cv2.waitKey(0)
        roi_mean = cv2.mean(frame_roi)

        if roi_mean[1] > 20 and roi_mean[1] < 50:  # s channel
            is_cal_box = True
...
while True:
    ...
    contours, _ = cv2.findContours(
        edged_frame, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    for i, contour in enumerate(contours):
        hull = cv2.convexHull(contour)
